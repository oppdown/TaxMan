...
const state = { store: null, view: 'dashboard', selectedYear: 2025, transactionDraft: null, phoneCapture: null, phonePairing: null, companionPairing: false, companionPairingData: null, qrScanner: false, qrScannerMessage: '', companionComputer: null, companionRequest: null, companionLastSent: false, companionAppendNext: false, pendingPhonePhoto: null, paymentModal: null, workUseModal: false, aboutOpen: false, shortcutsOpen: false, openMenu: null, ocr: null, appVersion: '0.4.7', search: '', typeFilter: 'all', categoryFilter: 'all', lastPdfPath: '', lastCsvPath: '', lastBackupPath: '' };
...
function renderTransactionForm() {
  ...
  return `<section class="panel form-panel"><div class="panel-body"><div class="form-title"><div><h2>${draft.id ? 'Edit transaction' : 'Add transaction'}</h2><p>Enter one income or expense item. Amounts are stored in U.S. dollars.</p></div><button class="icon-button" data-action="cancel-form" aria-label="Close form">✕</button></div><div class="entry-tip"><strong>Quick entry:</strong> Type dates as 090826. Ctrl+S saves, Esc cancels, and Ctrl+N starts a new expense.</div><form id="transaction-form"><div class="form-grid compact-form-grid">
    ...
    <div class="field wide"><label>Bill photo${images.length > 1 ? ` · ${images.length} pages` : ''}</label><div class="photo-actions">${photoButton}<label class="secondary-button file-button">Choose photo<input id="receipt-photo" type="file" accept="image/*" capture="environment"></label>${images.length && window.taxLedger.supportsOcr ? `<button type="button" class="secondary-button" data-action="read-bill-photo" ${state.ocr?.busy ? 'disabled' : ''}>${state.ocr?.busy ? 'Reading bill…' : 'Read bill details'}</button>` : ''}${images.length ? '<span class="photo-attached">Photo attached</span>' : '<span class="muted">Optional</span>'}</div>${pagePreview}${images.length ? `${state.ocr?.message ? `<div class="notice"><strong>${escapeHtml(state.ocr.message)}</strong> Verify every suggested field.${state.ocr.vendor && !draft.companyId ? ` <span class="ocr-vendor-suggestion">Possible company/source: <strong>${escapeHtml(state.ocr.vendor)}</strong> <button type="button" class="secondary-button compact-button" data-action="use-ocr-vendor">Create company from this name</button></span>` : ''}</div>` : ''}${state.ocr?.text ? `<details class="ocr-details"><summary>Show text read from bill</summary><pre>${escapeHtml(state.ocr.text)}</pre></details>` : ''}` : `<small>${window.taxLedger.supportsPhoneCapture ? 'Use the phone connection for a camera photo, or choose an image from this computer.' : 'Take a photo or choose an image. The bill stays on this device.'}</small>`}</div>
