...
test('native application menu contains the former in-app menu contents once', () => {
  ...
  assert.deepEqual(menu[0].submenu.filter((item) => item.label).map((item) => item.label), ['Save', 'Save As…', 'Choose workspace folder…', 'Open workspace folder', 'Exit']);
  ...
  assert.deepEqual(menu[3].submenu.map((item) => item.label), ['Check for Updates…', 'About', 'Keyboard shortcuts']);
  menu[3].submenu[0].click();
  menu[3].submenu[1].click();
  assert.deepEqual(actions, ['check-for-updates', 'show-about']);
  menu[0].submenu[2].click();
  assert.equal(actions[2], 'choose-workspace');
  menu[0].submenu[3].click();
  assert.equal(actions[3], 'open-workspace');
});
